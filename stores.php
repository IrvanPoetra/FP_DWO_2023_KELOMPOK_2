<?php include 'koneksi.php';
    ...
    $sql = "SELECT SUM(fp.OrderQty) AS tot from fakta_barang_tidak_diterima fp";
    $tot = mysqli_query($conn,$sql);
    $tot_amount = mysqli_fetch_row($tot);

    $sql = "SELECT 
    concat('name:',dl.province_name) AS name, 
    concat('y:',SUM(fp.OrderQty)*100/" . $tot_amount[0] .") AS y , 
    concat('drilldown:',dl.province_name) as drilldown 
    from 
        fakta_barang_tidak_diterima fp
        join vendor_address dl on fp.vendoraddress_id = dl.vendoraddress_id 
    WHERE 
        dl.vendoraddress_id >= 301
    group by name";
    ...
    $sql = "select dl.province_name as province, 
    SUM(fp.OrderQty) AS OrderQuantity
    from 
        fakta_barang_tidak_diterima fp
        join vendor_address dl on fp.vendoraddress_id = dl.vendoraddress_id 
    WHERE 
    ...
    $sql = "SELECT dl.province_name as Province, dl.city as Name, SUM(fp.OrderQty) AS OrderQuantity 
    from fakta_barang_tidak_diterima fp 
    join vendor_address dl on fp.vendoraddress_id = dl.vendoraddress_id 
    WHERE 
        dl.vendoraddress_id >= 301
    ...
    foreach($data_det[0] as $a) {
        ...
        if($i < count($data_det[0])-1) {
            if($a[0] != $data_det[0][$i+1][0]) {
                $denominator = cari_tot_kat($a[0], $tot_all_kat);
                $string_data .= '["' . $a[1] . '", ' . ($denominator != 0 ? $a[2]*100/$denominator : 0) . ']]},';
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">
                                                Jumlah Produk Dari Vendor
                                            </div>
                                            <div class="h5 mb-0 font-weight-bold text-gray-800">
                                                <?php                                           
                                                $sql = "SELECT COUNT(vendoraddress_id) as location from vendor_address";
...
Highcharts.chart('chart1', {
    ...
    title: {
        text: 'Persentase Produk Dibeli Berdasarkan Provinsi'
    },
    subtitle: {
