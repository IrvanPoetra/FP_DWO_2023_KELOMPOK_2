<?php include 'koneksi.php';
    ...
    $sql = "SELECT SUM(fp.order_qty) AS tot from fakta_pembelian fp";
    ...
    concat('y:',SUM(fp.order_qty)*100/" . $tot_amount[0] .") AS y , 
    concat('drilldown:',coalesce (pd.product_category, 'Tidak Memiliki Kategori')) as drilldown 
    from 
        fakta_pembelian fp
        join product pd on pd.product_id = fp.product_id 
        group by name";
    ...
    $sql = "SELECT coalesce (pd.product_category, 'Tidak Memiliki Kategori') AS ProductCategory, SUM(fp.order_qty) AS OrderQuantity 
    from fakta_pembelian fp join product pd on pd.product_id = fp.product_id 
    ...
    $sql = "SELECT coalesce (pd.product_category, 'Tidak Memiliki Kategori'), 
    pd.product_subcategory as Name, SUM(fp.order_qty) AS OrderQuantity 
    from fakta_pembelian fp join product pd on pd.product_id = fp.product_id 
                                    ...
                                    <div class="card-body">
                                        <div class="row no-gutters align-items-center">
                                            <div class="col mr-2">
                                                ...
                                                <div class="h5 mb-0 font-weight-bold text-gray-800">
                                                    <?php                                           
                                                    $sql = "SELECT SUM(order_qty) as jumlah_trbl from fakta_pembelian";
                                                    $query = mysqli_query($conn, $sql);
                                                    if (mysqli_num_rows($query) > 0){
                                                        while($row2=mysqli_fetch_array($query)){
...
Highcharts.chart('chart1', {
    ...
    xAxis: {
        categories : [
            <?php 
                $sql = "SELECT SUM(fp.order_qty) as Jumlah, dt.tahun AS Tahun from fakta_pembelian fp
                join time dt on dt.time_id = fp.time_id group by tahun";
                $query = mysqli_query($conn, $sql);
                if (mysqli_num_rows($query) > 0){
                    while($row2=mysqli_fetch_array($query)){
                        $th=$row2["Tahun"];
...
Highcharts.chart('chart2', {
    ...
    title: {
        text: 'Persentase Produk Dibeli Berdasarkan Kategori'
    },
    subtitle: {
